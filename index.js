...
const containerText = document.getElementById("quotes");
const authorText = document.getElementById("authorSection");
...
function newQuotes(){
    ...
    let quotess = apiQuotes[Math.floor(Math.random()*apiQuotes.length)];
...
authorText.innerText="Unknown";
   ...
   authorText.innerText= quotess.author;
}

// CSS property for textContent

if(quotess.text.length>50){

containerText.classList.add("longQuote");

}else{

    containerText.classList.remove("longQuote")
}

containerText.innerText = quotess.text;

}


// function for TwitterSection

function twitterSection(){

const twitterUrl = `https://twitter.com/intent/tweet?text=${containerText.textContent}-${authorText.text}`;
...
// EventListener for newQuotes

newQuoteButton.addEventListener("click",newQuotes);
twitterButton.addEventListener("click",twitterSection);
